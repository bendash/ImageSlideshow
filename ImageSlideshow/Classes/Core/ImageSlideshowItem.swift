    ...
    */
    ...
    override open func layoutSubviews() {
        ...
        if let caption = image.caption {
            let captionLabelHeight = (caption as NSString).boundingRect(with: CGSize(width: frame.size.width - 16, height: 0), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: captionLabel.font], context: nil).size.height + 8
            imageViewSize = CGSize(width: imageViewSize.width, height: imageViewSize.height-captionLabelHeight)
            captionLabelFrame = CGRect(x: 8, y: imageViewSize.height, width: frame.size.width - 16, height: captionLabelHeight)
        }
