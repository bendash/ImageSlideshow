...
open class ImageSlideshow: UIView {
    ...
    // Caption properties
    open var captionFont = UIFont.systemFont(ofSize: 11)
    open var captionColor = UIColor.black
    ...
    open func layoutPageControl() {
        ...
        // check if a caption is set anywhere in the images
        // and get the longest one
        let longestCaption = images.filter { (image) -> Bool in
            return image.caption != nil
        }.map { (image) -> String in
        ...
        if let caption = longestCaption {
            pageControlBottomInset += (caption as NSString).boundingRect(with: CGSize(width: frame.size.width - 16, height: 0), options: .usesLineFragmentOrigin, attributes: [NSAttributedStringKey.font: captionFont], context: nil).size.height + 12
        }
    ...
    func reloadScrollView() {
        ...
        for image in scrollViewImages {
            ...
            item.captionLabel.textColor = captionColor
